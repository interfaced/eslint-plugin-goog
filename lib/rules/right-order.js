...
const isGoogRequireExpression = require('../utils').isGoogRequireExpression;
const isGoogProvideExpression = require('../utils').isGoogProvideExpression;
const getGoogName = require('../utils').getNamespaceFromGoogExpression;
	...
	create: (context) => {
		...
		function reportUngroupedGoogNode(node) {
			...
			if (prevExpression) {
				...
				const comments = sourceCode.getComments(node.parent);
